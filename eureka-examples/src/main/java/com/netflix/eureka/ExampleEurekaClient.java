...
/**
 * todo 使用 Eureka 发现示例服务并发送请求的示例 Eureka 客户端。 在此示例中，程序尝试从 EurekaClient 获取示例，然后对支持的服务端点进行 REST 调用
...
public class ExampleEurekaClient {
    ...
    /**
     * todo 初始化info manager
     * @param instanceConfig 配置文件
     * @return applicationInfoManager
     */
    private static synchronized ApplicationInfoManager initializeApplicationInfoManager(EurekaInstanceConfig instanceConfig) {
        if (applicationInfoManager == null) {
            // todo 基于服务实例配置，构建instanceInfo服务实例
            InstanceInfo instanceInfo = new EurekaConfigBasedInstanceInfoProvider(instanceConfig).get();
            // todo 基于服务实例配置和instanceInfo服务实例，构建 applicationInfoManager实例管理器
    ...
    /**
     * todo eureka的启动类，这个类相当于自己写的一个普通的服务
     * @param args
     * @throws Exception
     */
    public static void main(String[] args) throws Exception{
        ...
        // todo new DefaultEurekaClientConfig()读取eureka-client.properties配置文件，形成eureka client的配置，基于接口对外提供eureka client配置的读取
        //      基于服务实例管理器和配置初始化EurekaClient【DiscoveryClient】
